...
class HttpRequest {
    ...
    ///   - failure: 连接服务器失败
    class func loadData<T: TargetType>(API: T.Type, target: T, cache: Bool = false, cacheHandle: ((Data) -> Void)? = nil, success: @escaping success, failure: failure ) {
        let provider = MoyaProvider<T>(plugins: [
            RequestHandlingPlugin()
            ])
        //let pro = MoyaProvider<T>()
        //如果需要读取缓存，则优先读取缓存内容
        ...
        provider.request(target) { result in
            ...
            case let .success(response):
                ...
                let mtJson : [Dictionary<String, Any>] = try! JSONSerialization.jsonObject(with: response.data, options: JSONSerialization.ReadingOptions.allowFragments) as! [Dictionary<String, Any>]
                print("mtJson ==\(mtJson)")
                
                let json = JSON(response.data)
                let jsonArray = try? response.mapJSON()
                let services:[MTResponse] = Mapper<MTResponse>().mapArray(JSONArray: try! response.mapJSON() as! [[String : Any]])
                
                let decoder = JSONDecoder()
                let serv = try? decoder.decode([MTServicesModel].self, from: response.data)
                
                let homeArray = try? decoder.decode(MTHomeHeader.self, from: response.data)
...
public extension TargetType {
    var baseURL: URL {
        return URL.init(string: "https://qzone.melonblock.com/app/")!
...
extension URLRequest {
    ...
    mutating func appendCommonParams() -> URLRequest {
        var request = try? encoded(parameters: commonParams, parameterEncoding: URLEncoding(destination: .queryString))
//        request?.timeoutInterval = 15.0
        request?.setValue("kkkkkkkkkkk", forHTTPHeaderField: "Authorization")
